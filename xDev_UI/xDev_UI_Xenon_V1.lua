				...
				if getsynasset then
					...
					elseif #image > 14 and string.sub(image, 1, 14) == "synasseturl://" then
						local x, e = pcall(function()
							...
							local fold = isfolder("./xDeveloper Lib")
							if not fold then
								makefolder("./xDeveloper Lib")
							end
							fold = isfolder("./xDeveloper Lib/Themes")
							if not fold then
								makefolder("./xDeveloper Lib/Themes")
							end
							fold = isfolder("./xDeveloper Lib/Themes/SynapseAssetsCache")
							if not fold then
								makefolder("./xDeveloper Lib Themes/SynapseAssetsCache")
							end
							if not fold or not isfile("./xDeveloper Lib/Themes/SynapseAssetsCache/" .. codename .. ".dat") then
								local res = game:HttpGet(string.sub(image, 15))
								if res ~= nil then
									writefile("./xDeveloper Lib/Themes/SynapseAssetsCache/" .. codename .. ".dat", res)
								end
							end
							return getsynasset(readfile("./xDeveloper Lib/Themes/SynapseAssetsCache/" .. codename .. ".dat"))
						end)
						if x and e ~= nil then
							return e
						end
...
function library:CreateWindow(options, ...)
	...
	if windowName and #windowName > 0 and library.WorkspaceName == "" then
		library.WorkspaceName = convertfilename(windowName, "xDeveloper Lib")
	end
		...
		function tabFunctions:CreateSection(options, ...)
			...
			if isfolder and makefolder and listfiles and readfile and writefile then
				function sectionFunctions:AddPersistence(options, ...)
					...
					local realDropdownHolderList = Instance_new("UIListLayout")
					local dropdownEnabled = false
					if not isfolder("./xDeveloper Lib") then
						makefolder("./xDeveloper Lib")
					end
					local common_string = "./xDeveloper Lib/" .. tostring(custom_workspace or library.WorkspaceName)
					local function resolvelist(nofold)
						if custom_workspace ~= options.Workspace then
							custom_workspace = options.Workspace
							common_string = "./xDeveloper Lib/" .. tostring(custom_workspace or library.WorkspaceName)
						end
	...
	function windowFunctions:CreateDesigner(options, ...)
		...
		local daaata = {{"AddTextbox", "__Designer.Textbox.ImageAssetID", backgroundsection, {
			...
			Workspace = "xDeveloper Lib Themes",
			Flags = flags,
			Suffix = "Theme",
			Desginer = true
...
local library = loadstring(game:GetObjects("rbxassetid://9197560384")[1].Source)()
